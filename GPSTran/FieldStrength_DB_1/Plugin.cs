...
namespace FieldStrength_DB_1
{
    public class Plugin:IPlugin
    {
        ...
        private FieldStrengthProtocol Pro;//使用场强协议，不用公用的协议
                        ...
                        lock (Resource.lckISSIOfEntity)
                        {
                            if (Resource.ISSIOfEntity.ContainsKey(i.ToString()))
                            {
                                if (Resource.ISSIOfEntity[i.ToString()].ContainsKey(temp.id))
                                {
                                    ...
                                    sql.AppendFormat("insert into {0}(ISSI,nMsRSSI,nULRSSI,nBattery,nReasonForSending,time)  values('{1}','{2}','{3}','{4}','{5}','{6}')\n", pModel.TableName, temp.id.PadLeft(12, '0'), temp.nMsRSSI, temp.nULRSSI, temp.nBattery, temp.nReasonForSending, temp.time);
                                    flag++;
                                    break;
                                }
                            }
        ...
        private int RepairTable(string name)
        {
            string sql = "CREATE TABLE " + pModel.TableName + "(" +
                         ...
                         "[ISSI] [varchar](20) NOT NULL," +
                         "[nMsRSSI] nvarchar(10) NULL," +
                         "[nULRSSI] nvarchar(10) NULL," +
                         "[nBattery] nvarchar(10) NULL," +
                         "[nReasonForSending] Varchar(max) NULL," +
