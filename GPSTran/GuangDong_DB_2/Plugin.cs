...
namespace GuangDong_DB_2
{
    public class Plugin : IPlugin
    {
                        ...
                        lock (Resource.lckISSIOfEntity)
                        {
                            if (Resource.ISSIOfEntity.ContainsKey(i.ToString()))
                            {
                                if (Resource.ISSIOfEntity[i.ToString()].ContainsKey(temp.id))
                                {
                                    ...
                                    sql.AppendFormat("insert into {0}(devid,longitude,latitude,SPEED,direction,locationtime) values('{1}',{2},{3},{4},{5},'{6}')\n", pModel.TableName, temp.id, temp.lon, temp.lat, temp.speed, temp.dir, temp.time);
                                    flag++;
                                    break;
                                }
                            }
        ...
        {
            //创建groupinfo
            //string sql = "CREATE TABLE [dbo].[GroupInfo]([ISSI] [int] primary key,[GSSI] [int] NULL,[WorkState] [int] NULL,[DailString] [int] default(0),[GroupDailString] [int] default(0))";
            //try
            //{
            ...
            sb.Append("select @nam=nam,@num=num,@entityname=entity_name,@type=[type],@lxdh=telephone from UserInfo B inner join inserted A on B.issi=A.devid; \n");
            ...
            sb.Append("select 1,'350M',A.devid,4412,A.longitude,A.latitude,A.SPEED,A.direction,A.locationtime,@clzl,@hphm,@num,@nam,@lxdh,@entitycode,@entityname,@Gssi from inserted A \n");
            sb.Append("end");
            try
            {
